...
title: "Part 5: Memory & State Management"
...
source: Notes
topic: Generative AI
coverImage: "/images/blog/genai/part-5-memory-state.png"
series: "Generative AI Engineering"
seriesPart: 5
seriesTotal: 8
---

*Generative AI Engineering · Part 5/8*

<Callout color="blue">
  Part 5 of the Generative AI Engineering series. If you're reading through top-to-bottom, start with [Part 1 — the cheatsheet](/blog/genai-interview-prep-cheatsheet) and read forward.
</Callout>
...
<Image src="/images/blog/genai/inline/inline-part-5-chat-buffer.png" alt="One conversation as a growing list of turns, with the context window bracketing the top 4 turns while older ones drop out, plus the messages JSON the API actually sees" wide />

<Image src="/images/blog/genai/inline/inline-part-5-context-overflow.png" alt="Five snapshots of the same 128k-token bar as history piles up — green with headroom, tight, at the limit, overflowing — plus the three ways overflow shows up in production" wide />
...
<Image src="/images/blog/genai/inline/inline-part-5-long-term-memory.png" alt="Same user across three sessions weeks apart — facts distilled in session A flow up into session B and C's system prompt, personalising without the user re-stating anything" wide />

<Image src="/images/blog/genai/inline/inline-part-5-fact-extraction.png" alt="One conversation, three facts distilled — chat log on the left, phrases highlighted, fact extractor LLM in the middle, dated fact cards stored on the right" wide />
...
<Image src="/images/blog/genai/inline/inline-part-5-session-state.png" alt="One user, many threads — user_id tree branching into independent thread IDs, each holding its own ordered messages + state + metadata" wide />
...
<Image src="/images/blog/genai/inline/inline-part-5-memory-types.png" alt="The four memory types every agent needs — working, episodic, semantic, procedural — as a 2×2 with the classic Python-data-structure analogy underneath" wide />
...
memory.save_context(
    {"input": "Maria is preparing for a an enterprise consultancy interview in Madrid."},
    {"output": "Good luck with the prep."},
)
# memory.entity_store now has:
# {"Alice": "Prefers espresso in the mornings.", "coffee": "...", "mornings": "..."}
```
...
<Image src="/images/blog/genai/inline/inline-part-5-semantic-cache.png" alt="Cache hit vs cache miss on the same-shape query — hit returns cached answer at 0ms/$0, miss forwards to the LLM — plus the threshold sweet-spot chart" wide />

<Image src="/images/blog/genai/inline/inline-part-5-cache-invalidation.png" alt="Three cache invalidation triggers (data changed, user changed, TTL expired), plus the TTL / version-key / manual-bust strategy triangle" wide />
...
<Image src="/images/blog/genai/inline/inline-part-5-history-strategies.png" alt="Full buffer (blows the window, red) vs sliding window (forgets earlier, slate-blue) vs summarising (compresses meaning, green) — plus the cost/faithfulness/quality tradeoff table" wide />
...
<Image src="/images/blog/genai/inline/inline-part-5-context-budget.png" alt="128k tokens allocated by role — system prompt, long-term memory, retrieved chunks, chat history, current message, reserved output — plus the growth curves showing history is what blows the budget" wide />
...
<Image src="/images/blog/genai/inline/inline-part-5-memory-agents.png" alt="Mem0, Zep, MemGPT side by side — each framework's memory architecture (fact extractor vs summariser+extractor vs OS-style paging) and which problem it solves" wide />

<Image src="/images/blog/genai/inline/inline-part-5-memgpt-paging.png" alt="MemGPT's two-tier memory — main context vs external context — with page_in and page_out tool calls, plus a ReAct trace showing the model deciding when to page" wide />
...
memory = Memory()
memory.add("Maria is preparing for a an enterprise consultancy interview in Madrid.", user_id="maria")

# Next session
results = memory.search("what is Maria working on?", user_id="maria")
# → returns the beverage preference fact
```
