...
Four engineering competencies I couldn't honestly claim before:

- **MLOps / model deployment.** Full pipeline from data to served model, with CI gating, retrain on every push, and automated rollouts to a live HTTPS endpoint.
- **Microsoft Azure.** Container Apps, Log Analytics workspace, role assignments, Consumption budgets — and the IAM model that holds it together.
- **Cloud migration.** End-to-end lift of a local scikit-learn model into a managed cloud service, with the deployment artifact reproducible from `git`.
- **CI/CD with secret-free auth.** GitHub Actions OIDC-federated to an Azure AD app registration — no long-lived secrets stored anywhere.

And one personal take-away that won't fit on a CV: **the model is the easy part**. Six minutes of every deploy is spent re-fitting the classifier. The other six weeks of work were getting the model to ship reliably. The bias of ML coursework — that the algorithm is the deliverable — is exactly backwards in practice.
