...
<Image src="/images/blog/genai/inline/inline-part-3-prompt-separators.png" alt="Messy free-text prompt vs the same prompt structured with delimited ROLE / TASK / DATA / OUTPUT blocks, and why separators help" wide />
...
<Image src="/images/blog/genai/inline/inline-part-3-debugging-bad-prompts.png" alt="The four ways prompts fail (ambiguity, missing context, conflicting instructions, open-ended output) and the 4-step debug loop" wide />

<Image src="/images/blog/genai/inline/inline-part-3-instruction-stack.png" alt="The instruction stack inside a chat — SYSTEM → USER → ASSISTANT — and who wins when they conflict" wide />

## 2. Task decomposition

<Image src="/images/blog/genai/inline/inline-part-3-task-decomposition.png" alt="One monolithic prompt vs the same task decomposed into 4 small sequential prompts, plus the decomposition tree — leaves are LLM calls, everything above is code" wide />
...
<Image src="/images/blog/genai/inline/inline-part-3-cot-vs-zeroshot.png" alt="Same maths problem: zero-shot answers 27 (wrong) vs chain-of-thought showing the 3 reasoning lines that arrive at 11 (correct), plus the GSM8K accuracy jump" wide />
...
<Image src="/images/blog/genai/inline/inline-part-3-self-consistency.png" alt="One question, five reasoning chains — 4 arrive at 11 (green), 1 at 9 (red) — with the vote counter, plus the accuracy jump from single CoT to self-consistency@k=5" wide />
...
<Image src="/images/blog/genai/inline/inline-part-3-tot.png" alt="Branching + evaluating + backtracking — a small ToT search tree with per-node scores, best path in green, pruned branches in red — plus the Game-of-24 accuracy where ToT (74%) crushes CoT (4%)" wide />
...
<Image src="/images/blog/genai/inline/inline-part-3-react-loop.png" alt="ReAct end-to-end: Question → Thought → Action (tool call) → Observation → Answer, plus why interleaving cognition and tool use beats either alone" wide />
...
<Image src="/images/blog/genai/inline/inline-part-3-planner-executor.png" alt="Planner (slow, cheap-token model) writes the numbered plan; Executor (fast, real-tool model) runs it — plus the full loop with a 'replan on failure' feedback arrow" wide />
...
<Image src="/images/blog/genai/inline/inline-part-3-prompt-iteration.png" alt="How good prompts are actually built — the hypothesis → write → test → inspect → refine loop, plus the metrics to measure while iterating" wide />
...
<Image src="/images/blog/genai/inline/inline-part-3-technique-decision-tree.png" alt="A decision tree for prompt design — six branches from 'What kind of task is this?' to Zero-shot, Few-shot, CoT, RAG, Tool use/ReAct, Planner-Executor — plus the quick cheat sheet" wide />

<Image src="/images/blog/genai/inline/inline-part-3-technique-matrix.png" alt="The 2×2 that decides your technique — task complexity × confidence needed → Zero-shot / CoT / ReAct / ToT+reflection — plus the cost-vs-quality scatter of each technique" wide />
